...
import { StudentRestService } from '../service/student-rest.service';
    ...
    class StudentComponent {
      ...
      doUpdate(updatedStudent: Student) {
        // First, update the student in the database by making a call to the service
        this.service.updateStudent(updatedStudent).subscribe(
          response => {
            // If the update is successful, update the local list of students
            let modifiedStudents = this.students.map(s => {
              if (s.rollno == updatedStudent.rollno) {
                return {...s, numberOfAttempts: updatedStudent.numberOfAttempts, percentage: updatedStudent.percentage, subjectsLearning: updatedStudent.subjectsLearning};
              } else {
                return s;
              }
            });
